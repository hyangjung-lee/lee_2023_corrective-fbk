function recover_bads_BMBU(subIDtoFit, recovInfo, funInputFurther, sim_path)
...
prior_mu = [0.5, 0,    3,   3, 3, 3            ];
prior_var = [0.1,0.25,  1.5, 1.5, 1.5, 1.5     ];

guessIn_LB   = [0.1,  -2,         0   ,      0 ,           0             0 ];
guessIn_UB   = [2.5,    2,       8,        8,            8,            8   ];
...
options_ibs.Vectorized = false; 
                ...
                for iSim = 1: numSim
                    ...
                    R = acrR(:, iSim); 
                    llfun = @(theta) ibslike(@fitting_BMBU, theta, R, fInput.designM, options_ibs, fInput); 
